...
Transcrypts operates across three environments:

- **Sandbox**: For development with predefined sample payroll provider data
...
| `access_secret` | Environment-specific private key | `sandbox-a57b3109f1f4a8b3f2ebbc1c526950f1795464af` |

The `client_id` remains constant across environments. The `access_secret` differs for each environment with prefixes: `sandbox`, `dev`, or `prod`.
