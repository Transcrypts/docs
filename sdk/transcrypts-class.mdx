...
<Warning>
The `verify` method validates the `origin` of all incoming `postMessage` events against the configured `widgetUrl` origin. Messages from unrecognized origins are silently ignored.
</Warning>
...
<Steps>
  <Step title="Create session token">
    A base64-encoded session token is created containing `apiKey`, `ssn`, `user`, and `timestamp`.
  </Step>
  <Step title="Mount iframe overlay">
    An iframe overlay is created and mounted to `document.body`.
  </Step>
  <Step title="Load widget">
    The iframe loads `{widgetUrl}/verify?token={token}`.
  </Step>
  <Step title="Listen for events">
    The SDK listens for `message` and `keydown` (Escape) events on `window`.
  </Step>
  <Step title="Settle promise">
    When a terminal event is received (`verification_complete`, `verification_failed`, or `close`), the iframe overlay is destroyed and the promise settles.
  </Step>
</Steps>
...
Subscribes a callback function to a specific widget event type. Multiple callbacks can be registered for the same event type. Events are emitted as the widget communicates via `postMessage`.
